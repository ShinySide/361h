...
static void sprdfb_dispc_clean_lcd (struct sprdfb_device *dev)
{
	...
	/*Config img layer*/
	reg_value = (1<<0); /*enable img layer*/
	reg_value |= (0<<4);/*YUV422*/
	dispc_write(reg_value, DISPC_IMG_CTRL);
	dispc_write(dev->fb->fix.smem_start, DISPC_IMG_Y_BASE_ADDR);
	dispc_write(dev->fb->fix.smem_start, DISPC_IMG_UV_BASE_ADDR);
	/*reg_value = dispc_read(DISPC_OSD_SIZE_XY);*/
	size = (dev->panel->width &0xffff) | (0x1<<16);
	dispc_write(size, DISPC_IMG_SIZE_XY);
	/*dispc_write(reg_value, DISPC_IMG_SIZE_XY);*/
	reg_value = dispc_read(DISPC_OSD_PITCH);
	/*dispc_write(dev->panel->width &0xffff, DISPC_IMG_PITCH);*/
	dispc_write(reg_value, DISPC_IMG_PITCH);
